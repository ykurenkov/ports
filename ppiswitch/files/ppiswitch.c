...
#define HELP_TARGET stdout;
    ...
    (void)fprintf(stderr, "\t--device device,\n\t-d device\t- parallel port device, default /dev/ppi0.\n");
    (void)fprintf(stderr, "\t--quiet,\n\t-q\t\t- supress output.\n");
    (void)fprintf(stderr, "\t--on <channels>,\n\t-i <channels>\t- turn on <channels>.\n");
    (void)fprintf(stderr, "\t--off <channels>,\n\t-o <channels>\t- turn off <channels>.\n");
    (void)fprintf(stderr, "\t--kick <channels>,\n\t-k <channels>\t- kick on/off or off/on <channel>\n\t\t\t  according current status.\n");
    (void)fprintf(stderr, "\t--time <sec>,\n\t-t <sec>\t- kick timeout in sec (1-86400), 1 sec default.\n");
    (void)fprintf(stderr, "\t--alloff,\n\t-r\t\t- turn off all channels.\n");
    (void)fprintf(stderr, "\t--allon,\n\t-s\t\t- turn on all channels.\n");
    (void)fprintf(stderr, "\t--status,\n\t-c\t\t- show turned on channels.\n");
...
int main(int argc, char* argv[]) {
    ...
    if ( argc > 1 ) {
        ...
        while ( (ch=getopt_long(argc,argv,"qd:t:i:k:o:v:rscgh", longopts, NULL)) != EOF)
            switch (ch) {
                ...
                case 't': {
                        ...
                        if (*ep || ep == optarg || ultmp > 86400 || ! ultmp)
